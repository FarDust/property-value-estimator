...
def evaluate_model_component(
...
) -> NamedTuple('EvaluationOutputs', [('metrics', Metrics), ('feature_importance', Dataset), ('residuals_chart', HTML), ('predictions_chart', HTML), ('feature_importance_chart', HTML)]): # type: ignore
    """
    ...
    import pandas as pd
    import cloudpickle
    from property_value_estimator.model.model.evaluation import ModelEvaluator, ChartConfig
    ...
    metrics_artifact = Metrics(
        uri=dsl.get_uri(suffix='metrics'),
    ...
    figures_dict['prediction_vs_actual'].write_html(predictions_html.path, include_plotlyjs='cdn')
    
    feature_importance_html = HTML(uri=dsl.get_uri(suffix='feature_importance_chart.html'))
    figures_dict['feature_importance'].write_html(feature_importance_html.path, include_plotlyjs='cdn')
    
    # Create the output namedtuple dynamically
    EvaluationOutputs = NamedTuple('EvaluationOutputs', [('metrics', Metrics), ('feature_importance', Dataset), ('residuals_chart', HTML), ('predictions_chart', HTML), ('feature_importance_chart', HTML)])
